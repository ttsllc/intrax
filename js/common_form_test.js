...
function prog_check() {
  ...
  $("input:checkbox[name='prog[data][]']:checked").each(function () {
	...
	if ($(this).val() == "アユササマーキャンプ") ayusa_summer = true;
...
function prog_check_confirm() {
  ...
  if ($("input:hidden[name='setsumei_hope[data]']").val() == "希望する") {
    ...
    ary_prog.forEach(function (val, index, ary_prog) {
	  ...
	  if (val == "アユササマーキャンプ"){$("tr.summer_camp_seminar_date").show();}
    });
  }
...
function kari_check() {
  ...
  if ($('input:checkbox[name="aupair_kari[data][]"]').is(':checked')) {
    // チェックボックスがチェックされている場合の処理
    console.log('チェックボックスがチェックされています');
  } else {
    // チェックボックスがチェックされていない場合の処理
    console.log('チェックボックスがチェックされていません');
  }
